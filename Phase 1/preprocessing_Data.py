...
def fill_Null_train(data):
    # features(lat, lng)(only contain a default missing value), (Negative_Review, Positive_Review )
    ...
    if P_rows_with_nan < 0.07:
        ...
        data = data.fillna({'Hotel_Address': 'no address', 'Additional_Number_of_Scoring': 0.0,
                            'Day': "00", 'Month': "00", 'Year': "0000", 'Average_Score': X3, 'Hotel_Name': 'no name',
                            ...
                            'days_since_review': 0.0, 'lat': X14, 'lng': X15, 'Reviewer_Score': 0.0})
    # value that will restore in file
    values = {'Average_Score': X3, 'Total_Number_of_Reviews': X8, 'lat': X14, 'lng': X15}
    # pickle
    with open('values', mode='bw') as f:
        pickle.dump(obj=values, file=f)
    return data


#########################################################################
def fill_Null_test(data):
    # load store values from file
    with open('values', mode='br') as f:
    ...
    if P_rows_with_nan < 0.07:
        ...
        X3 = values['Average_Score']
        X8 = values['Total_Number_of_Reviews']
        X14 = values['lat']
        X15 = values['lng']
